#!/usr/bin/env node
/**
 * Clanker Alignment Pipeline
 * 
 * Complete end-to-end fee claim → rebalance → burn → treasury flow using Bankr API.
 * No private keys needed — all transactions signed and submitted via Bankr.
 * 
 * Pipeline:
 ...
 * 
 * Usage:
 *   node alignment.mjs --token 0xf3ce5ddaab6c133f9875a4a46c55cf0b58111b07 \
...
import { createPublicClient, http, formatEther, formatUnits, parseEther, maxUint256, parseAbi, encodeFunctionData } from 'viem';
...
const argv = yargs(hideBin(process.argv))
  ...
  .option('bankr-key', { type: 'string', description: 'Bankr API key (or set BANKR_API_KEY env var)' })
...
class BankrClient {
  ...
  async getWalletAddress() {
    ...
    const response = await fetch(`${this.baseUrl}/agent/sign`, {
      method: 'POST',
      headers: {
    ...
    this.walletAddress = data.signer;
    return this.walletAddress;
  }

  async submitTransaction(to, data, value = '0', description = '', waitForConfirmation = true) {
    const response = await fetch(`${this.baseUrl}/agent/submit`, {
      method: 'POST',
      headers: {
        'X-API-Key': this.apiKey,
        'Content-Type': 'application/json',
 ...
 */
async function claimFees({ token, publicClient, bankrClient, dryRun }) {
  console.log(`\n📋 Step 1: Claim Fees`);
  console.log(`═`.repeat(50));

  const walletAddress = await bankrClient.getWalletAddress();
  ...
  try {
    wethFees = await publicClient.readContract({
      ...
      args: [walletAddress, CONTRACTS.WETH],
    });
  } catch (e) {
    wethFees = await publicClient.readContract({
  ...
  if (dryRun) {
    console.log(`\n[DRY] Would claim fees via Bankr API`);
  ...
  if (wethFees > 0n) {
    ...
    wethTx = await bankrClient.submitTransaction(
      CONTRACTS.CLANKER_FEE,
      claimData,
      '0',
      'Claim WETH fees from Clanker'
    );
  ...
  let tokenTx = null;
  if (tokenFees > 0n) {
    console.log(`\n⏳ Claiming ${tokenSymbol} fees via Bankr...`);
    const claimData = encodeFunctionData({
      abi: CLANKER_FEE_ABI,
      functionName: 'claim',
      args: [walletAddress, token],
    });
    tokenTx = await bankrClient.submitTransaction(
      CONTRACTS.CLANKER_FEE,
      claimData,
      '0',
      `Claim ${tokenSymbol} fees from Clanker`
    );
    console.log(`   ✅ https://basescan.org/tx/${tokenTx}`);
    await sleep(1000);
  }

  return { wethFees, tokenFees, tokenSymbol, tokenDecimals, wethTx, tokenTx, walletAddress };
}
 ...
 */
async function rebalance({ token, tokenSymbol, tokenDecimals, wethPrice, tokenPrice, poolKey, publicClient, bankrClient, walletAddress, dryRun }) {
  console.log(`\n⚖️  Step 3: Rebalance to 50/50`);
  console.log(`═`.repeat(50));

  // Get current balances
  const wethBal = await retry(() => publicClient.readContract({
    address: CONTRACTS.WETH, abi: ERC20_ABI, functionName: 'balanceOf', args: [walletAddress],
  }));
  const tokenBal = await retry(() => publicClient.readContract({
    address: token, abi: ERC20_ABI, functionName: 'balanceOf', args: [walletAddress],
  }));
  ...
  if (dryRun) {
    console.log(`\n[DRY] Would execute V4 swap via Bankr API`);
 ...
 */
async function burnTokens({ token, tokenSymbol, tokenDecimals, publicClient, bankrClient, walletAddress, dryRun }) {
  console.log(`\n🔥 Step 4: Burn Tokens`);
  console.log(`═`.repeat(50));
  ...
  if (dryRun) {
    console.log(`\n[DRY] Would burn ${formatUnits(tokenBal, tokenDecimals)} ${tokenSymbol} to 0xdead via Bankr`);
    return { amount: tokenBal, txHash: null };
  }

  console.log(`\n⏳ Burning ${formatUnits(tokenBal, tokenDecimals)} ${tokenSymbol} via Bankr...`);
  const transferData = encodeFunctionData({
    abi: ERC20_ABI,
    functionName: 'transfer',
    args: [CONTRACTS.DEAD, tokenBal],
  });
  const txHash = await bankrClient.submitTransaction(
    token,
    transferData,
    '0',
    `Burn ${formatUnits(tokenBal, tokenDecimals)} ${tokenSymbol} tokens`
  );
  console.log(`   ✅ https://basescan.org/tx/${txHash}`);
 ...
 */
async function treasurySplit({ token, treasury, publicClient, bankrClient, walletAddress, dryRun }) {
  ...
  if (dryRun) {
    console.log(`\n[DRY] Would swap ${formatEther(half)} WETH → USDC via Bankr`);
    console.log(`[DRY] Would swap ${formatEther(half)} WETH → BNKR via Bankr`);
    console.log(`[DRY] Would send USDC + BNKR to ${treasury} via Bankr`);
    return { usdcAmount: 0n, bnkrAmount: 0n };
  }
  ...
  const allowance = await retry(() => publicClient.readContract({
    ...
    args: [walletAddress, CONTRACTS.SWAP_ROUTER_02],
  }));
  if (allowance < wethBal) {
    console.log(`\n⏳ Approving WETH → SwapRouter02 via Bankr...`);
    const approveData = encodeFunctionData({
      abi: ERC20_ABI,
      functionName: 'approve',
      args: [CONTRACTS.SWAP_ROUTER_02, maxUint256],
    });
    await bankrClient.submitTransaction(
      CONTRACTS.WETH,
      approveData,
      '0',
  ...
  console.log(`\n⏳ Swapping ${formatEther(half)} WETH → USDC via Bankr...`);
  const usdcSwapData = encodeFunctionData({
    abi: SWAP_ROUTER_ABI,
  ...
  const usdcTx = await bankrClient.submitTransaction(
  ...
  console.log(`\n⏳ Swapping ${formatEther(half)} WETH → BNKR via Bankr...`);
  const bnkrSwapData = encodeFunctionData({
    abi: SWAP_ROUTER_ABI,
    functionName: 'exactInputSingle',
    args: [{
      tokenIn: CONTRACTS.WETH,
  ...
  const bnkrTx = await bankrClient.submitTransaction(
    CONTRACTS.SWAP_ROUTER_02,
    bnkrSwapData,
    '0',
    'Swap WETH to BNKR'
  ...
  const usdcBal = await retry(() => publicClient.readContract({
    address: CONTRACTS.USDC, abi: ERC20_ABI, functionName: 'balanceOf', args: [walletAddress],
  }));
  const bnkrBal = await retry(() => publicClient.readContract({
    address: CONTRACTS.BNKR, abi: ERC20_ABI, functionName: 'balanceOf', args: [walletAddress],
  }));

  // Send to treasury
  console.log(`\n⏳ Sending ${formatUnits(usdcBal, 6)} USDC to treasury via Bankr...`);
  const usdcSendData = encodeFunctionData({
    abi: ERC20_ABI,
    functionName: 'transfer',
    args: [treasury, usdcBal],
  });
  const usdcSendTx = await bankrClient.submitTransaction(
    CONTRACTS.USDC,
    usdcSendData,
    '0',
    `Send ${formatUnits(usdcBal, 6)} USDC to treasury`
  );
  console.log(`   ✅ https://basescan.org/tx/${usdcSendTx}`);

  console.log(`\n⏳ Sending ${formatUnits(bnkrBal, 18)} BNKR to treasury via Bankr...`);
  const bnkrSendData = encodeFunctionData({
    abi: ERC20_ABI,
    functionName: 'transfer',
    args: [treasury, bnkrBal],
  });
  const bnkrSendTx = await bankrClient.submitTransaction(
    CONTRACTS.BNKR,
    bnkrSendData,
    '0',
    `Send ${formatUnits(bnkrBal, 18)} BNKR to treasury`
  );
...
async function ensureApprovals(token, amount, publicClient, bankrClient, walletAddress) {
  // ERC20 → Permit2
  const erc20Allowance = await retry(() => publicClient.readContract({
    address: token, abi: ERC20_ABI, functionName: 'allowance',
    args: [walletAddress, CONTRACTS.PERMIT2],
  }));
  if (erc20Allowance < amount) {
    console.log(`   Approving ${token} → Permit2 via Bankr...`);
    const approveData = encodeFunctionData({
      abi: ERC20_ABI,
      functionName: 'approve',
  ...
  const [permit2Amount] = await retry(() => publicClient.readContract({
    ...
    args: [walletAddress, token, CONTRACTS.UNIVERSAL_ROUTER],
  }));
  if (BigInt(permit2Amount) < amount) {
    console.log(`   Approving Universal Router on Permit2 via Bankr...`);
...
async function executeV4Swap({ poolKey, zeroForOne, amountIn, minAmountOut, bankrClient, walletAddress }) {
  ...
  const hash = await bankrClient.submitTransaction(
...
async function main() {
  const bankrApiKey = argv.bankrKey || process.env.BANKR_API_KEY;
  if (!bankrApiKey) {
    console.error('❌ BANKR_API_KEY not set (use --bankr-key or env var)');
    process.exit(1);
  }

  const bankrClient = new BankrClient(bankrApiKey);
  const rpcUrl = process.env.BASE_RPC_URL || 'https://mainnet.base.org';
  const publicClient = createPublicClient({ chain: base, transport: http(rpcUrl) });

  const { token, treasury, currency0, fee, tickSpacing, hooks, dryRun } = argv;

  console.log(`🎯 Clanker Alignment Pipeline (Bankr Edition)`);
  console.log(`═`.repeat(50));
  ...
  try {
    // Pipeline execution
    const claimResult = await claimFees({ token, publicClient, bankrClient, dryRun });
