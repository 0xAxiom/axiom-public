...
function x402Gate(config) {
    ...
    return async (req, res, next) => {
        ...
        const paymentHeader = req.headers['x-payment-hash'];
        if (!paymentHeader) {
        ...
        const expectedPayment = {
            amount: (0, verify_js_1.parsePriceToUsdc)(routeConfig.price),
            currency: 'USDC',
        ...
        try {
            const verification = await (0, verify_js_1.verifyPayment)(paymentHeader, expectedPayment, facilitatorUrl);
 ...
 */
function findMatchingRoute(routes, requestRoute) {
 ...
 */
function matchesWildcard(pattern, route) {
 ...
 */
function sendPaymentRequired(res, routeConfig, wallet, chainId, usdcAddress, facilitatorUrl) {
    const amount = (0, verify_js_1.parsePriceToUsdc)(routeConfig.price);
    const priceDisplay = (0, verify_js_1.formatUsdcToPrice)(amount);
